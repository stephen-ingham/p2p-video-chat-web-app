...
- `web-server/src`: `npm run dev` runs Astro directly (`astro dev`); `npm run build` / `npm run preview` for production builds. `npm test` (or `npm run test:component` from the repo root) runs Vitest via `web-server/src/vitest.config.ts` (Astro's `getViteConfig` integration, jsdom environment). Tests live in `web-server/tests/unit/` (token-worker/TokenService, CSP middleware — real coverage) and `web-server/tests/components/` (planned component tests, still empty scaffolding). `web-server/tests/` has its own `package.json`/`node_modules` (testing-library, a type-only copy of `astro`) and its own `tsconfig.json`, separate from `web-server/src/` — same sibling-`node_modules` pattern as `web-socket-api/tests/`, needed because it sits outside `web-server/src/`'s dependency tree.
