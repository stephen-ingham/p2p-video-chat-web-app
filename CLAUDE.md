...
- a regional external Application Load Balancer (`/auth/*`, `/call/*`, `/wss/*` → backend, everything else → frontend, HTTP → HTTPS redirect) with a DNS-validated Certificate Manager certificate
- a self-hosted coturn STUN/TURN VM

Required stack config: `gcp:project`, `domain` (also sets the backend's `ALLOWED_ORIGIN`), and the secrets. Each stack deploys into its own GCP project: the proxy-only subnet can only exist once per region per network, so the two stacks can't share one. The VM (`infra/turn-server.ts`) is only created when `voneo-video-chat:turnEnabled` is true, which today is the prod stack only. It's an `e2-micro` Container-Optimized OS instance with a static IP, a firewall rule, and its own service account that can read only the `turn-secret-<stack>` secret. It runs the pinned `coturn/coturn` image with `infra/coturn/turnserver.conf` plus prod-only lines (external IP, deny relaying to private/metadata ranges). Keep that image tag in sync with `e2e/compose.nat.yaml`. The Pulumi snapshot test enables TURN, so it covers these resources. Separate `dev`/`prod` stacks (`infra/Pulumi.dev.yaml` / `Pulumi.prod.yaml`, GCP resource/secret names suffixed by stack). Deploy/destroy via `npm run gcp-deploy-dev` / `npm run gcp-destroy-dev` / `npm run gcp-deploy-prod` / `npm run gcp-destroy-prod` (run `pulumi up`/`pulumi destroy --stack <dev|prod>` from `infra/`). In CI, `deploy-dev.yml`/`deploy-prod.yml` run the same on merge to `dev`/`main` via GCP Workload Identity Federation. None of this is set up yet: there are no GCP projects, Pulumi stacks, or GitHub environment secrets, so nothing has been deployed. `infra/README.md` walks through the setup.

## Known incomplete areas

- Calls and their WebSocket servers are in-memory only; nothing survives an API restart.
- Production CORS/WS-origin allowlist is a single `ALLOWED_ORIGIN` value, set from each Pulumi stack's `domain` config. No real domain has been chosen yet.
