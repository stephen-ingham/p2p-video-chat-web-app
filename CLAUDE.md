...
- Tests: `npm test` in `mobile-app/` runs Jest (`jest-expo`) + React Native Testing Library over `mobile-app/tests/`; Maestro flows in `mobile-app/.maestro/` (run locally with `npm run test:maestro` from the repo root, which needs the dev build on a running emulator; `mobile-e2e.yml` builds a release APK with `expo prebuild` + Gradle and runs them on an emulator). `tests/setup.ts` stubs safe-area insets and reduced motion with plain functions, because the tests' `jest.resetAllMocks()` wipes the libraries' own `jest.fn()` mocks.
